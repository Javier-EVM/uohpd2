...
def loadData(dataname, binary = False):
    # balance-scale
    if binary == True:
        if dataname == 'house-votes-84':
            x, y = loadHouseVotes84_binary()
            return x, y
        
        if dataname == 'spambase':
            x, y = loadSpambase_binary()
            return x, y
        
        if dataname == "Monks1":
            x,y = loadMonks1_binary()
            return x, y
        ...
        if dataname == "Credit-approval":
            x,y = loadCreditapproval_binary()
            return x, y
        
        if dataname == "Ionosphere":
            x,y = loadIonosphere_binary()
            return x, y
        
        if dataname == "Adult":
            x,y = loadAdult_binary()
            return x, y
        if dataname == "Musk-2":
            x,y = loadMusk2_binary()
            return x, y
        
        if dataname == "Connectionist-bench":
            x,y = loadConnectionistB_binary()
            return x, y
        
        if dataname == "Breast-cancer":
            x,y = loadBreastCancer_binary()
            return x, y
        
        if dataname == "Pima-diabetes":
            x,y = loadPimaDiabetes_binary()
            return x,y
        
        if dataname == "Statlog":
            x,y = loadStatlog_binary()
            return x,y
        
        if dataname == "Banknote":
            x,y = loadBanknote_binary()
            return x,y
        
        if dataname == "Bank-marketing":
            x,y = loadBankMarketing_binary()
            return x,y
    else:
        if dataname == 'house-votes-84':
...
# |          |   |          |   |                   |                     
# |          |   |          |   |                   |                          
#  \________/  o |_________/  o |_________ o        |


def loadBankMarketing_binary():
    df = pd.read_csv("./data/bankmarketing/bank.csv", delimiter=';')
    dfx = df.iloc[:, :-1]  # Todas las columnas excepto la última como características
    ...
    col_cat = dfx.select_dtypes(include=['object','category']).columns
    for col in col_num:
        mediana = statistics.median(dfx[col])
        dfx[col] = dfx[col].apply(lambda x: 0 if x < mediana else 1)
    dfx = pd.get_dummies(dfx, columns=col_cat, prefix=col_cat,drop_first = True)
    col_bool = dfx.select_dtypes(include=['boolean']).columns
    le = LabelEncoder()
    for col in col_bool:
...
def loadBanknote_binary():
    ...
    '''
    dfx = np.array(dfx)
    return dfx , np.array(dfy)

def loadStatlog_binary():
    ...
    '''
    df = pd.read_csv("./data/statlog/german_num.data-numeric", header=None,delim_whitespace=True)# delimiter=',')
    dfx = df.iloc[:, :-1].copy()# Todas las columnas excepto la última como características
    for col in dfx.columns:
        dfx[col] = dfx[col].apply(lambda x: 0 if x < statistics.median(dfx[col]) else 1)
    dfy = df.iloc[:, -1]   # Última columna como etiquetas
    dfy_map = {1:1,2:0} 
    '''
    se hace un proceso similar a replace, pero con una idea de "mapeo" con .map() 
...
def loadBreastCancer_binary():
    df = pd.read_csv("./data/breast-cancer/brca.csv")
    dfx = df.iloc[:, :-1]  # Todas las columnas excepto la última como características
    dfy = df.iloc[:, -1]   # Última columna como etiquetas
    for col in dfx.columns:
        median = statistics.median(dfx[col])
        dfx[col] = dfx[col].apply(lambda x: 0 if x < median else 1 )
    dfx = np.array(dfx)
    '''
    Se codifica la última columna a 0s y 1s
    '''
...
def loadMusk2_binary():
    ...
    col_num = dfx.select_dtypes(include=['int64', 'float64']).columns
    col_cat = dfx.select_dtypes(include=['object','category','boolean']).columns
    for col in col_num:
        median = statistics.median(dfx[col])
        dfx[col] = dfx[col].apply(lambda x: 0 if x < median else 1 )
    dfx = pd.get_dummies(dfx, columns=col_cat, prefix=col_cat,drop_first = True)
...
def loadIonosphere_binary():
    ...
    # print(dfx)
    dfy = df[34]
    dfy = dfy.replace({"g": 1, "b": 0}) #df.loc[:, 0:56], : toma todas las filas, 0:56 toma las columnas de 0 a 56, contando 56
    return np.array(dfx), np.array(dfy)

def loadCreditapproval_binary():
    """
    Ejemplo de linea de datos
    b,34.83,4,u,g,d,bb,12.5,t,f,0,t,g,?,0,-
...
def loadMonks1_binary():
    ...
    x = np.concatenate((x_train, x_test), axis=0)
    y = np.concatenate((y_train, y_test), axis=0)
    dfx = pd.DataFrame(x, columns = df_train[[2,3,4,5,6,7]].columns )
    dfy = pd.DataFrame(y, columns = df_train[[1]].columns)
    for col in dfx.columns:
        median = statistics.median(dfx[col])
        dfx[col] = dfx[col].apply(lambda x: 0 if x < median else 1 )
    # print(dfx)
